...
module StorageFileApiHelper =
    ...
    let parseSearchOptions (path: Path option) (searchOptions: SearchOptions Option): Map<string, obj> =
    ...
    let moveOrCopy<'T> (fromPath: Path) (toPath: Path) (action: string)
                       (storageFileApi: StorageFile): Async<Result<'T, StorageError>> =
        match action with
        | "move" | "copy" ->
            let body =
                Map<string, string>
        ...
        | _ ->
            async {
                return Error { message = $"Unsupported action {action}, use move/copy action!"
                               statusCode = None }
            }
        
    /// Adds key-value pair to map if value is given
    let addTransformValueIfPresent (key: string) (value: 'a option) (map: Map<string, string>): Map<string, string> =
        match value with
        | Some v -> map |> Map.add key (v.ToString().ToLower())
        | _      -> map
    
    /// Gets Map representation of optional transform options 
    let getTransformOptionsParamsMap (transform: TransformOptions option): Map<string, string> =
    ...
    let getFullFilePath (bucketId: string) (path: string): string = $"{bucketId}/{path}"
...
module StorageFileApi =
    ...
    let list (path: Path option) (searchOptions: SearchOptions option)
             (storageFileApi: StorageFile): Async<Result<FileObject list, StorageError>> =
        ...
        async {
            ...
            return deserializeResponse<FileObject list> response
        }
    
    /// Moves file from given location to given location
    let move (fromPath: Path) (toPath: Path) (storageFileApi: StorageFile): Async<Result<MessageResponse, StorageError>> =
        moveOrCopy<MessageResponse> fromPath toPath "move" storageFileApi
            
    /// Copies file from given location to given location
    let copy (fromPath: Path) (toPath: Path) (storageFileApi: StorageFile): Async<Result<FileResponse, StorageError>> =
    ...
    let createSignedUrl (path: string) (expiresIn: int<s>) (transform: TransformOptions option)
                        (storageFileApi: StorageFile): Async<Result<string, StorageError>> =
    ...
    let createSignedUrls (paths: Path list) (expiresIn: int<s>)
                         (storageFileApi: StorageFile): Async<Result<string list, StorageError>> =
        let body =
            Map<string, obj>
                [ "expiresIn", expiresIn
                  "paths", paths ]
        ...
        async {
            ...
            return
                match deserializedResponse with
                | Ok r    -> Ok (r |> List.map (fun url -> $"{storageFileApi.connection.Url}{url.signedUrl}"))
                | Error e -> Error e
        }
    
    /// Downloads file at given given path with given transform options
    let download (path: Path) (transform: TransformOptions option)
                 (storageFileApi: StorageFile): Async<Result<byte[], StorageError>> =
    ...
    let getPublicUrl (path: Path) (transform: TransformOptions option) (storageFileApi: StorageFile): string =
    ...
    let remove (paths: Path list) (storageFileApi: StorageFile): Async<Result<FileObject list, StorageError>> =
    ...
    let upload (path: Path) (file: byte[]) (storageFileApi: StorageFile): Async<Result<FileResponse, StorageError>> =
        ...
        async {
            ...
            return deserializeResponse<FileResponse> response
        }
        
    /// Replaces file at given path by given file
    let update (path: Path) (file: byte[]) (storageFileApi: StorageFile): Async<Result<FileResponse, StorageError>> =
        let content = new ByteArrayContent(file)
